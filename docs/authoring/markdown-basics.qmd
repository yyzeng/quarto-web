...
Examples above are using the shorter form when you only set a language for highlighting. Longer form using language as class (i.e. `.python`) inside brackets can also be used and allows to add attributes to the block of source code (like with @sec-divs-and-spans). Some specific features are using this syntax like @sec-line-numbers or @sec-code-filename. Here is an example for the latter:
